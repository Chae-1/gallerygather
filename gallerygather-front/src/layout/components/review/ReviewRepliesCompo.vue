...
export default {
  ...
  methods: {
    ...
    async getExhibitReviews() {
      axios
        .get(
          `http://localhost:8080/api/exhibition/${this.exhibitionId}/review/${this.reviewId}/replies?page=${this.currentPage - 1}&size=${this.perPage}&sort=regDate,desc`
        )
        .then((response) => {
    ...
    async addReply() {
      ...
      if (store.loginCheck) {
        ...
        try {
          apiRequest(
            'post',
            `http://localhost:8080/api/exhibition/review/${this.reviewId}/replies`,
            {
    ...
    async deleteReply(replyId) {
      ...
      if (store.loginCheck) {
        ...
        try {
          await apiRequest(
            'delete',
            `http://localhost:8080/api/exhibition/reviews/${this.reviewId}/replies/${replyId}`
          ).then((response) => {
            if (response.status == 201) {
              this.getExhibitReviews()
            } else {
